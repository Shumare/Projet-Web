...
?>
<h1 class="text-center">Modification du profil de
        <?= $people->people_firstname ?>
</h1>
<form class="row g-3" action="/account/update" method="POST">
        ...
        <div class="col-md-6">
                ...
                <select class="form-select" name="people_gender" aria-label="selection role" required>
                        <option <?php if ($people->people_gender == "Male") {
                                echo "selected";
                        }
                        ; ?> value="Male">Male
                        </option>
                        <option <?php if ($people->people_gender == "Female") {
                                echo "selected";
                        }
                        ; ?> value="Female">Female
                        </option>
        ...
        <div class="col-md-6">
                ...
                <input type="text" class="form-control" name="acc_email"
                        value="<?= $account->getAcc_email_From_Id($people->id) ?>" required>
        </div>
        <div class="col-md-6">
                ...
                if ($_SESSION['user_role'] == 'admin') { ?>
                        <select class="form-select" name="id_role" aria-label="selection role" required>
                                <option <?php if (($account->getId_role($people->id))[0]->id_role == "3") {
                                        echo "selected";
                                }
                                ; ?> value="3">Student
                                </option>
                                <option <?php if (($account->getId_role($people->id))[0]->id_role == "2") {
                                        echo "selected";
                                }
                                ; ?> value="2">Tutor</option>
                                <option <?php if (($account->getId_role($people->id))[0]->id_role == "1") {
                                        echo "selected";
                                }
                        ...
                        <select class="form-select" name="none" aria-label="select user role" disabled>
                                <option <?php if (($account->getId_role($people->id))[0]->id_role == "3") {
                                        echo "selected";
                                }
                                ; ?> value="3">Student
...
<a href="/account/delete/<?= $people->id ?>" onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce compte ?')"
        class="link-danger link-offset-2 link-underline-opacity-25 link-underline-opacity-100-hover btn btn-outline-danger btn-sm">supprimer
        <?= $people->people_firstname ?></a>